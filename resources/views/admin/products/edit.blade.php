    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('locations') ? 'is-invalid' : '' }}" name="locations[]" id="locations" multiple required>
                    @foreach($locations as $id => $location)
                        <option value="{{ $id }}" {{ (in_array($id, old('locations', [])) || $product->locations->contains($id)) ? 'selected' : '' }}>{{ $location }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $product->quantity) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ipaddress') ? 'is-invalid' : '' }}" type="text" name="ipaddress" id="ipaddress" value="{{ old('ipaddress', $product->ipaddress) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('serialnumber') ? 'is-invalid' : '' }}" type="text" name="serialnumber" id="serialnumber" value="{{ old('serialnumber', $product->serialnumber) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ $product->status || old('status', 0) === 1 ? 'checked' : '' }} required>
